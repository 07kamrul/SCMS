...
| File | Action | Why |
|---|---|---|
| `docker-compose.yml` | UPDATE | Add top-level `name: scfms` to pin the Compose project namespace (containers/network/volumes) regardless of directory name; parameterize all 5 host-side port mappings as `${SCFMS_*_PORT:-<non-common-default>}` |
| `.env.example` (new, repo root) | CREATE | Document the new `SCFMS_DB_PORT`, `SCFMS_REDIS_PORT`, `SCFMS_MINIO_PORT`, `SCFMS_MINIO_CONSOLE_PORT`, `SCFMS_BACKEND_PORT` vars Compose reads for host-port substitution |
...
- **Action**: Add `name: scfms` as the first line of `docker-compose.yml`, above `services:`
- **Mirror**: N/A — new top-level Compose key, no existing pattern in this repo to mirror
- **Validate**: `docker compose config | head -5` shows `name: scfms`

### Task 2: Parameterize host ports with non-colliding defaults
- **Action**: Change each service's host-side port to `${SCFMS_<SERVICE>_PORT:-<default>}`, using defaults outside the common range other local stacks are likely to claim, e.g.:
  - db: `"${SCFMS_DB_PORT:-15432}:5432"`
  - redis: `"${SCFMS_REDIS_PORT:-16379}:6379"`
  - minio: `"${SCFMS_MINIO_PORT:-19000}:9000"`, `"${SCFMS_MINIO_CONSOLE_PORT:-19001}:9001"`
  - backend: `"${SCFMS_BACKEND_PORT:-18000}:8000"`
...
- **Validate**: `docker compose up -d` then `docker compose ps` shows the new default host ports bound; re-run with `SCFMS_BACKEND_PORT=28000 docker compose up -d` and confirm the backend answers on `28000` instead

### Task 3: Document the new vars in a root `.env.example`
- **Action**: Create `/.env.example` at repo root (sibling to `docker-compose.yml`) listing the 5 `SCFMS_*_PORT` vars with their defaults and one line each on purpose; note real `.env` (gitignored) is what Compose actually reads for substitution
...
SCFMS_BACKEND_PORT=28000 docker compose up -d backend && curl -sf http://localhost:28000/api/v1/health
...
|---|---|---|
| Existing local `.env` files or scripts hardcode the old ports (5432/6379/9000/8000) | Medium | Defaults only change for *new* setups; anyone with an existing root `.env` can keep old values by setting `SCFMS_*_PORT` explicitly — call this out in `.env.example` |
